...
let modalHeight =
  Platform.OS === 'ios' ? 400 * em : StatusBar.currentHeight + 400 * em;
...
class BasicPicker extends Component {
  ...
  _confirmChose() {
    this.props.data.map((item, index) => {
      this.str =
        this.str +
        this.props.data[index][this.select.selectedValue[index]].name;
    });
  ...
  _setEventBegin() {
    ...
    if (this.props.enable) {
      ...
      this.str = '';
      return {
        valueCount: this.select.selectIndex,
        indexCount: this.select.selectedValue,
      };
    } else {
  ...
  _setModalVisible(visible, type) {
    if (visible) {
      ...
      Animated.timing(this.state.animatedHeight, {toValue: height}).start(() =>
        this._changeAnimateStatus(type),
      );
    }
  }
      ...
      <View style={styles.container}>
        <Modal
          ...
          <View style={[styles.modalContainer]}>
            <Animated.View
              style={[styles.innerContainer, {top: this.state.animatedHeight}]}>
              <Handle
