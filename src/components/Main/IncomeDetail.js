...
import {Icon, ArrowBackIcon} from 'native-base';
import {
  BackHandler,
  Image,
  Dimensions,
...
class IncomeDetail extends Component {
  ...
  componentWillMount() {
    ...
    if (Global.saveData.prePage == 'Profile') {
      ...
      Global.saveData.prevUrl = url;
      Global.saveData.preOtherId = otherId;
      Global.saveData.prename = name;
      Global.saveData.preage = age;
      Global.saveData.preimage = imageUrl;
      Global.saveData.prematchID = mid;
      Global.saveData.preuserdistance = parseInt(distance);
      ...
      this.setState({
        ...
        userimage: imageUrl,
        matchId: mid,
        userdistance: parseInt(distance),
    ...
    if (isNaN(value)) {
      ...
      if (value > Global.saveData.coin_count) {
        this.setState({
          ...
          sendDiamondsCount: value,
          msgError:
            'You can send only ' + Global.saveData.coin_count + ' diamonds.',
        });
      } else {
        this.setState({
          errorMsg: false,
        ...
        <View style={{position: 'absolute', left: 0, bottom: 40}}>
          <View
            ...
            <View style={{flexDirection: 'row'}}>
              ...
              <Image
                source={diamond}
                style={{marginLeft: 10, width: 15, height: 15, marginTop: 2}}
              />
