...
import {Text, Icon, ArrowBackIcon} from 'native-base';
import {
  Dimensions,
  TextInput,
  ScrollView,
...
class ProfileSetting extends Component {
  ...
  get_block = () => {
    var itmes = [];
    var data = ['Yes - ON', 'No - OFF'];
    for (var i = 0; i < 2; i++) {
      itmes.push({value: data[i]});
    }
    this.setState({
      auto_block: Global.saveData.auto_block == 1 ? 'Yes - ON' : 'No - OFF',
      blockData: itmes,
    });
  };
  onBack() {
  ...
  onUpdate() {
    ...
    if (
      this.state.coin_per_message == '' ||
      this.state.coin_per_message == null
    ) {
      ...
      .then(responseJson => {
        if (!responseJson.error) {
          ...
          alert_str +=
            ...
            resultData.user_id +
            ' Confirmation Code ' +
            resultData.confirmation_code;
          alert_str +=
            ' In your email, please describe why you believe this was done in error.';
            ...
            <View>
              <Dropdown
                ...
                onChangeText={country =>
                  this.setState({country, disabled: false})
                }
          ...
          <View
            ...
            <View>
              ...
              <Text style={{color: '#808080', fontSize: 10, marginTop: 10}}>
                {'Automatically block users sending inappropriate chats'}
              </Text>
            </View>
            <View>
              <Dropdown
                ...
                onChangeText={auto_block =>
                  this.setState({auto_block, disabled: false})
                }
