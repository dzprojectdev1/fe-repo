...
class Profile extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      coin_per_message: data.coin_per_message,
      fullImage: false,
      flash_ban: false,
  ...
  componentDidMount() {
    ...
    const othername = this.state.otherData.name;
    ...
    if (isNaN(sendDiamondsCount)) {
      ...
      if (sendDiamondsCount > Global.saveData.coin_count) {
        Alert.alert(
          ...
          'You only have ' +
            Global.saveData.coin_count +
            ' diamonds available. You need more diamonds.',
          [
            {
              text: 'Cancel',
            ...
            if (!responseJson.error) {
              if (responseJson.data.account_status == 1) {
                if (responseJson.data.sending_available) {
                  ...
                  this.setState({
                    coin_count:
                      parseInt(this.state.coin_count) +
        ...
        .then(responseJson => {
          if (!responseJson.error) {
            ...
            if (row.contentType == 2) {
              var v_url =
                `${SERVER_URL}/api/storage/videoLink?fileId=` + row.imgUrl;
              fetch(v_url, {
                method: 'GET',
                headers: {
                  'Content-Type': 'application/json',
                    ...
                    data: {
                      imageUrl:
                        row.imgUrl !== '' && row.imgUrl !== null
                          ? GCS_BUCKET + row.imgUrl + '-screenshot'
                          : null,
                      videoUrl: responseJson.url,
      ...
      <ImageBackground source={bg} style={{width: '100%', height: '100%'}}>
        ...
        <Modal
          ...
          <View style={{width: DEVICE_WIDTH, height: DEVICE_HEIGHT}}>
            <TouchableOpacity onPress={() => this.setState({fullImage: false})}>
              <Image
          ...
          <View style={styles.screenOverlay}>
            <View style={styles.dialogPrompt}>
              <View
                ...
                />
                <Text style={{marginTop: 20, color: '#000'}}>{'mutual'}</Text>
              </View>
              <Text style={[styles.bodyFont, {color: '#000'}]}>
          ...
          <View>
            <Image
              ...
              style={{
                ...
                zIndex: 100,
                position: 'absolute',
                left: parseInt(DEVICE_WIDTH / 2) - 150,
                top: parseInt(DEVICE_HEIGHT / 2) - 150,
              }}
            />
          </View>
        ) : null}
            ...
            <View
              ...
              <Text
                style={{
                  fontSize: 12,
                  color: '#7d7d7d',
                }}>{`${this.state.coin_per_message} per message`}</Text>
            </View>
          </View>
                ...
                return (
                  <TouchableOpacity
                    ...
                    <View
                      ...
                      <Text style={{fontSize: 16, color: '#000'}}>
                        {parseInt(index) +
                          parseInt(this.state.fanUsersCount) +
                          1 +
                          '.'}
                      </Text>
                    </View>
