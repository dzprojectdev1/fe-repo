...
import {ArrowBackIcon, Text} from 'native-base';
...
class ScreenGpay01 extends Component {
  ...
  timer = () => {
    const newCount = this.state.remainTimeStamp - 1;
    if (newCount >= 0) {
  ...
  goNext = () => {
    const responseReceipt = this.state.receipt;
    const responseProductId = JSON.parse(responseReceipt).productId;
    let responseOrderId = JSON.parse(responseReceipt).orderId;
    ...
    formBody.push(
      'package_name' + '=' + JSON.parse(responseReceipt).packageName,
    );
    formBody.push(
      'acknowledge' + '=' + JSON.parse(responseReceipt).acknowledge,
    );
    formBody.push('order_id' + '=' + JSON.parse(responseReceipt).orderId);
    formBody.push('product_id' + '=' + JSON.parse(responseReceipt).productId);
    formBody.push(
      'developer_payload' + '=' + JSON.parse(responseReceipt).developerPayload,
    );
    formBody.push(
      'purchase_time' + '=' + JSON.parse(responseReceipt).purchaseTime,
    );
    formBody.push(
      'purchase_state' + '=' + JSON.parse(responseReceipt).purchaseState,
    );
    formBody.push(
      'purchase_token' + '=' + JSON.parse(responseReceipt).purchaseToken,
    );
        ...
        if (responseJSON.error === false) {
          ...
          if (responseJSON.data.validation) {
            ...
            Alert.alert(
              'Success!',
              ' ' +
                diamondCount[productIdIndex] +
                ' diamonds were added to your account.',
              [{text: 'OK', onPress: () => console.log('OK Pressed')}],
              {cancelable: false},
            );
  ...
  goBack() {
    if (
      ...
      Global.saveData.nowPage == 'ChatScreen'
    ) {
  ...
  gotoFreeDiamonds = () => {
    const user_id = Global.saveData.u_id;
            ...
            if (responseData.success == false) {
              Alert.alert(
                ...
                'Please wait ' +
                  ...
                  ' seconds to unlock 30 free diamonds.',
                [{text: 'OK', onPress: () => console.log('OK Pressed')}],
                {cancelable: false},
              );
            } else {
  ...
  render() {
    const {productList, productListPass, receipt, availableItemsMessage} =
      this.state;
      ...
      <View style={styles.contentContainer}>
        ...
        <View>
          <View style={{justifyContent: 'center', alignItems: 'center'}}>
            <View
              ...
              <Image
                ...
                style={{width: 25, height: 25, marginTop: 22, marginLeft: 10}}
              />
              <Text
                style={{
                  color: '#cc2e48',
                ...
                return (
                  <TouchableOpacity
                    ...
                    <View style={{flexDirection: 'row', paddingTop: 18}}>
                      ...
                      />
                      <Text
                        style={{color: '#000', fontSize: 12, marginLeft: 10}}>
                        {this.treatProductTitle(product.title)}
                      </Text>
              ...
              <View style={styles.list_item_normal_last}>
                <View style={{flexDirection: 'row', paddingTop: 10}}>
                  ...
                  <Text
                    ...
                    {'Unlimited Instant Chat ends in'}
                  </Text>
                </View>
                <View style={{flexDirection: 'row', paddingTop: 18}}>
                  <Text style={{color: '#000', fontSize: 12, marginLeft: 35}}>
                    <Text style={{color: '#000', fontSize: 12, marginLeft: 35}}>
                      {this.state.remainDays != 0
                        ? this.state.remainDays + ' days '
                        : ''}
                    </Text>
                    <Text style={{color: '#000', fontSize: 12, marginLeft: 35}}>
                      {this.state.remainHours != 0
                        ? this.state.remainHours + ' hours '
                        : ''}
                    </Text>
                    <Text style={{color: '#000', fontSize: 12, marginLeft: 35}}>
                      {this.state.remainMinutes != 0
                        ? this.state.remainMinutes + ' minutes '
                        : ''}
                    </Text>
                    <Text style={{color: '#000', fontSize: 12, marginLeft: 35}}>
                      {this.state.remainSeconds != 0
                        ? this.state.remainSeconds + ' seconds '
                        : ''}
                    </Text>
                  </Text>
                </View>
          ...
          <View
            ...
            pointerEvents={this.state.free_button_condition ? 'auto' : 'none'}>
            <TouchableOpacity onPress={() => this.gotoFreeDiamonds()}>
              <View
                style={
                  this.state.free_button_condition
                    ? styles.free_diamond_button
                    : styles.free_diamond_button_disabled
                }>
...
const styles = StyleSheet.create({
  ...
  top_title: {
    ...
    alignItems: 'center',
    width: '100%',
    height: Platform.select({android: 60, ios: 40}),
    backgroundColor: '#fff',
  },
